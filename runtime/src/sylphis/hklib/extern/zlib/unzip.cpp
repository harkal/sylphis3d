...
int  Unz_GenCache(unz_s *s)
{
	...
	filecache=(cacheelem_t *)malloc(sizeof(cacheelem_t)*(filecount));
	...
	s->cache=(cacheelem_t **)malloc(sizeof (cacheelem_t*) *HASHSIZE);
    for (i=0;i<HASHSIZE;i++){
		s->cache[i]=(cacheelem_t *)malloc(sizeof(cacheelem_t)*amounts[i]);
...
void Unz_GetStringForDir (unzFile * pak,const string &path,const string &extension, vector<CFile> &fr)
{
	unz_s *s=(unz_s*) pak;
	char * token;
	int i,j,found=0;
	...
	for (i=0;i<HASHSIZE;i++)
	{
		for (j=0;j<s->counts[i];j++)
		{
			...
			if (cpath[0])
			if (strncmp (token, cpath, strlen (cpath)))
				continue;

			if (cpath[0])
				if (strlen (token) <= strlen(cpath) + extension.length())
					continue;

			if (strlen (token) > extension.length())
				if (!strncmp(extension.c_str(), token + strlen(token) - extension.length(), extension.length())){
				//	if (cpath)
				//		token += strlen(cpath);
					fr.push_back(CFile(token));
				}
		}
	}
...
int Unz_FileExists (unzFile * pak, const char * file )
{
	...
	for (i=0;i<HASHSIZE ;i++ ){
		for (j=0;j<s->counts[i];j++){
			if (!strcasecmp (file,s->cache[i][j].name))
				return 1;
		}
	}
                                  ...
                                  int iCaseSensitivity)
{
    ...
    key=hashstring(szFileName) % HASHSIZE;

    for (i=0;i<s->counts[key];i++){
        if (!strcasecmp(szFileName,s->cache[key][i].name)){
