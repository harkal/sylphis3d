...
static void doCleanupLogOnFirstRun()
{
    if (cleanupLogOnFirstRun) {
        ...
        log("--------------------------------------------------------------------------------");
        log("");
        log("      %s - Memory logging file created on %s", memoryLogFile,
            asctime(localtime(&t)));
        log("--------------------------------------------------------------------------------");
        log("");
                               ...
                               const char *sourceFunc)
{
    ...
    sprintf(str, "%s(%05d)::%s", sourceFileStripper(sourceFile), sourceLine,
            sourceFunc);
    return str;
}
...
static sAllocUnit *findAllocUnit(const void *reportedAddress)
{
    ...
    unsigned int hashIndex =
        (reinterpret_cast <
         unsigned int >(const_cast <
                        void *>(reportedAddress))>>4) &(hashSize - 1);
...
static void *calculateReportedAddress(const void *actualAddress)
{
    ...
    return reinterpret_cast < void *>(const_cast <
                                      char *>(reinterpret_cast <
                                              const char *>(actualAddress) +
                                              sizeof(long) * paddingSize));
}

// ---------------------------------------------------------------------------------------------------------------------------------

static void wipeWithPattern(sAllocUnit * allocUnit, unsigned long pattern,
    ...
    if (alwaysWipeAll && allocUnit->reportedSize > originalReportedSize) {
        ...
        long *lptr =
            reinterpret_cast < long *>(reinterpret_cast <
                                       char *>(allocUnit->reportedAddress) +
                                       originalReportedSize);
        int length =
            static_cast <
            int >(allocUnit->reportedSize - originalReportedSize);
        int i;
        ...
        for (i = 0; i < (length & 0x3); i++, cptr++, shiftCount += 8) {
            *cptr =
                static_cast <
                char >((pattern & (0xff << shiftCount)) >> shiftCount);
        }
    }
...
static void dumpAllocations(FILE * fp)
{
    ...
    for (unsigned int i = 0; i < hashSize; i++) {
        ...
        while (ptr) {
            fprintf(fp,
                    "%06d 0x%08X 0x%08X 0x%08X 0x%08X 0x%08X %-8s    %c       %c    %s\r\n",
                    ptr->allocationNumber,
                    reinterpret_cast < unsigned int >(ptr->reportedAddress),
                    ptr->reportedSize,
                    reinterpret_cast < unsigned int >(ptr->actualAddress),
                    ptr->actualSize, m_calcUnused(ptr),
...
void *operator new[] (size_t reportedSize)
{
    ...
    for (;;) {
        ...
        void *ptr =
            m_allocator(file, line, func, m_alloc_new_array, reportedSize);
        if (ptr) {
#ifdef TEST_MEMORY_MANAGER
            log("[D] EXIT : new[]");
#endif
...
void *operator new(size_t reportedSize, const char *sourceFile,
                   int sourceLine)
{
#ifdef TEST_MEMORY_MANAGER
    log("[D] ENTER: new");
    ...
    for (;;) {
        ...
        void *ptr = m_allocator(sourceFile, sourceLine, "??", m_alloc_new,
                                reportedSize);
        if (ptr) {
#ifdef TEST_MEMORY_MANAGER
            log("[D] EXIT : new");
                      ...
                      int sourceLine)
{
    ...
    for (;;) {
        ...
        void *ptr =
            m_allocator(sourceFile, sourceLine, "??", m_alloc_new_array,
                        reportedSize);
        if (ptr) {
#ifdef TEST_MEMORY_MANAGER
            log("[D] EXIT : new[]");
...
void operator delete(void *reportedAddress)
{
    ...
    if (reportedAddress)
        m_deallocator(sourceFile, sourceLine, sourceFunc, m_alloc_delete,
                      reportedAddress);
    else if (alwaysLogAll)
        log("[-] ----- %8s of NULL                      by %s",
            allocationTypes[m_alloc_delete], ownerString(sourceFile,
                                                         sourceLine,
                                                         sourceFunc));
...
void operator delete[] (void *reportedAddress)
{
    ...
    if (reportedAddress)
        m_deallocator(sourceFile, sourceLine, sourceFunc,
                      m_alloc_delete_array, reportedAddress);
    else if (alwaysLogAll)
        log("[-] ----- %8s of NULL                      by %s",
            allocationTypes[m_alloc_delete_array], ownerString(sourceFile,
                                                               sourceLine,
                                                               sourceFunc));
...
void *m_allocator(const char *sourceFile, const unsigned int sourceLine,
                  const char *sourceFunc, const unsigned int allocationType,
    ...
    try {
        ...
        if (alwaysLogAll)
            log("[+] %05d %8s of size 0x%08X(%08d) by %s",
                currentAllocationCount, allocationTypes[allocationType],
                reportedSize, reportedSize, ownerString(sourceFile,
                                                        sourceLine,
                                                        sourceFunc));
        ...
        if (!reservoir) {
            ...
            sAllocUnit **temp = (sAllocUnit **) realloc(reservoirBuffer,
                                                        (reservoirBufferSize +
                                                         1) *
                                                        sizeof(sAllocUnit *));
            m_assert(temp);
        ...
        if (sourceFile)
            strncpy(au->sourceFile, sourceFileStripper(sourceFile),
                    sizeof(au->sourceFile) - 1);
        else
        ...
        unsigned int hashIndex =
            (reinterpret_cast <
             unsigned int >(au->reportedAddress) >> 4) &(hashSize - 1);
        ...
        if (alwaysLogAll)
            log("[+] ---->             addr 0x%08X",
                reinterpret_cast < unsigned int >(au->reportedAddress));
...
void *m_reallocator(const char *sourceFile, const unsigned int sourceLine,
                    const char *sourceFunc,
                    const unsigned int reallocationType,
                    const size_t reportedSize, void *reportedAddress)
{
    try {
#ifdef TEST_MEMORY_MANAGER
        ...
        if (!reportedAddress) {
            return m_allocator(sourceFile, sourceLine, sourceFunc,
                               reallocationType, reportedSize);
        }
        // Increase our allocation count
        ...
        if (alwaysLogAll)
            log("[~] %05d %8s of size 0x%08X(%08d) by %s",
                currentAllocationCount, allocationTypes[reallocationType],
                reportedSize, reportedSize, ownerString(sourceFile,
                                                        sourceLine,
                                                        sourceFunc));
        ...
        unsigned int originalReportedSize =
            static_cast < unsigned int >(au->reportedSize);

        if (alwaysLogAll)
            log("[~] ---->             from 0x%08X(%08d)",
                originalReportedSize, originalReportedSize);
        ...
        if (oldReportedAddress != au->reportedAddress) {
...

            {
                unsigned int hashIndex =
                    (reinterpret_cast <
                     unsigned int >(oldReportedAddress) >> 4) &(hashSize - 1);
        ...
        if (stats.totalActualMemory > stats.peakActualMemory)
            stats.peakActualMemory = stats.totalActualMemory;
        int deltaReportedSize =
            static_cast < int >(reportedSize - originalReportedSize);
...
void m_deallocator(const char *sourceFile, const unsigned int sourceLine,
                   const char *sourceFunc,
                   ...
                   const void *reportedAddress)
{
    try {
        ...
        if (alwaysLogAll)
            log("[-] ----- %8s of addr 0x%08X           by %s",
                allocationTypes[deallocationType],
                reinterpret_cast < unsigned int >(const_cast <
        ...
        m_assert((deallocationType == m_alloc_delete
                  && au->allocationType == m_alloc_new)
                 || (deallocationType == m_alloc_delete_array
                     && au->allocationType == m_alloc_new_array)
                 || (deallocationType == m_alloc_free
...
bool m_validateAllocUnit(const sAllocUnit * allocUnit)
{
    ...
    long *post =
        reinterpret_cast <
        long *>((char *) allocUnit->actualAddress + allocUnit->actualSize -
                paddingSize * sizeof(long));
...
unsigned int m_calcUnused(const sAllocUnit * allocUnit)
{
    const unsigned long *ptr =
        reinterpret_cast < const unsigned long *>(allocUnit->reportedAddress);
...
{
    log("[I] %sAddress (reported): %010p", prefix,
        allocUnit->reportedAddress);
    log("[I] %sAddress (actual)  : %010p", prefix, allocUnit->actualAddress);
    log("[I] %sSize (reported)   : 0x%08X (%s)", prefix,
        static_cast < unsigned int >(allocUnit->reportedSize),
        memorySizeString(static_cast <
                         unsigned int >(allocUnit->reportedSize)));
    log("[I] %sSize (actual)     : 0x%08X (%s)", prefix,
        static_cast < unsigned int >(allocUnit->actualSize),
        memorySizeString(static_cast <
                         unsigned int >(allocUnit->actualSize)));
    log("[I] %sOwner             : %s(%d)::%s", prefix, allocUnit->sourceFile,
        allocUnit->sourceLine, allocUnit->sourceFunc);
    log("[I] %sAllocation type   : %s", prefix,
        allocationTypes[allocUnit->allocationType]);
